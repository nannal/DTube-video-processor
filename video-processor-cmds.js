...
var cmds = {
	...
	ipfs_cmds: {
		...
		ipfsUpload: (filePath, justHash, prop) => {
			...
			const ipfs = ipfsAPI(ipfsIp, ipfsPort, {
	...
	checkIfFinished: (filePath) => {
		...
		var func = setInterval(() => {
			...
			if (cmds.encoderResponse.ipfsAddSourceVideo.hash && cmds.encoderResponse.sprite.ipfsAddSprite.hash && encodedVidsHash.every((hash) => {
					return hash
				})) {
				clearInterval(func)
